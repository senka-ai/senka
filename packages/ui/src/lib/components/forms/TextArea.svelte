<script lang="ts">
  import type { FormInputComponent } from '@ui/type/component'
  ...
  import { FormRenderer } from '@ui/utils/rendering'
  ...
  {#if showCustomHelperArea}
    <div class="mt-1 flex items-start justify-between">
      {#if (effectiveError || helperText) && showHelperText}
        <div class="text-body-s {effectiveError ? 'text-error-400' : 'text-neutral-600'}">
          {effectiveError || helperText}
        </div>
      {/if}
      {#if showCharCount}
        <div class="text-body-s ml-auto text-neutral-600">
          {charCount}/{maxlength}
        </div>
      {/if}
    </div>
