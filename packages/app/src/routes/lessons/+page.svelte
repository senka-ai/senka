<script lang="ts">
  ...
  function getStatusBadge(status: Lesson['status']): { variant: BadgeVariant; text: string } {
    switch (status) {
      case 'new':
        return { variant: 'default', text: 'Nou' }
      case 'in_progress':
        return { variant: 'warning', text: 'În progres' }
      case 'completed':
    ...
    {#each lessons as lesson}
      <div class="card">
        <div class="lesson-header">
          ...
          <Badge type="dot" variant={getStatusBadge(lesson.status).variant} size="small" />
        </div>
        ...
        <div class="lesson-actions">
          {#if lesson.status === 'new'}
            <Button variant="primary" size="small" onclick={() => startLesson(lesson.id)}>
          ...
          {:else if lesson.status === 'in_progress'}
            ...
            <Button variant="tertiary" size="small" onclick={() => completeLesson(lesson.id)}>
