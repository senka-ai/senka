<script lang="ts">
  ...
  let inputClasses = $derived.by(() => {
    const base = 'w-full px-3 py-2 text-body-l bg-white border rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-1';
    
    const states = {
      default: 'border-[var(--color-neutral-300)] focus:border-[var(--color-highlight-400)] focus:ring-[var(--color-highlight-200)]',
      focused: 'border-[var(--color-highlight-400)] ring-2 ring-[var(--color-highlight-200)]',
      error: 'border-[var(--color-error-200)] focus:border-[var(--color-error-200)] focus:ring-[var(--color-error-100)]',
      disabled: 'border-[var(--color-neutral-300)] bg-[var(--color-neutral-100)] text-[var(--color-neutral-500)] cursor-not-allowed'
    };
...
<div class={containerClasses}>
  ...
  <div class="relative">
    {#if leftIcon && showIcon}
      <div class="absolute left-3 top-1/2 transform -translate-y-1/2 text-[var(--color-neutral-500)]">
        {leftIcon}
      </div>
    {/if}
    ...
    {#if rightIcon && showIcon}
      <div class="absolute right-3 top-1/2 transform -translate-y-1/2 text-[var(--color-neutral-500)]">
        {rightIcon}
      </div>
    {:else if unit && showUnit}
      <div class="absolute right-3 top-1/2 transform -translate-y-1/2 text-[var(--color-neutral-600)] text-body-m">
        {unit}
      </div>
    {/if}
  </div>
