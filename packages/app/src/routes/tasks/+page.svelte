<script lang="ts">
  ...
  function getStatusBadge(status: TaskStatus): { variant: BadgeVariant; text: string } {
    switch (status) {
      case 'pending':
        return { variant: 'warning', text: 'În așteptare' }
      case 'in_progress':
        return { variant: 'default', text: 'În progres' }
      case 'completed':
        return { variant: 'success', text: 'Completat' }
    }
  }
  ...
  function getTypeBadge(type: TaskType) {
    switch (type) {
      case 'homework':
        return { icon: '📝', text: 'Temă' }
      case 'test':
        return { icon: '📋', text: 'Test' }
      case 'project':
        return { icon: '🔬', text: 'Proiect' }
      case 'reading':
        return { icon: '📚', text: 'Lectură' }
    }
  }
    ...
    {#each tasks as task}
      <div class="card">
        <div class="task-header">
          ...
          <Badge type="dot" variant={getStatusBadge(task.status).variant} size="small" />
        </div>
        ...
        <div class="task-footer">
          ...
          <div class="task-actions">
            <Button variant="tertiary" size="small" onclick={() => toggleTaskStatus(task.id)}>
              {#snippet children()}
